...
void onDataReceive(const esp_now_recv_info_t *info, const uint8_t *incomingData, int len) {
  if (len != sizeof(SensorData)) {
    Serial.println("⚠️ Received data size mismatch!");
    return;
  }
  memcpy(&receivedData, incomingData, sizeof(receivedData));
  Serial.print("New Altitude: ");
  Serial.println(newAltitudeSlave);
  distanceGround=newAltitudeSlave-receivedData.altitude_m;
  Serial.print("Distance to ground: ");
  Serial.println(distanceGround);
}
void checkDistance(void* params){
  while(1){

    newAltitudeSlave = bmp.readAltitude(1013.25);  // Read the altitude (in meters)
